...
export const emailHistory = pgTable(
  'email_history',
  {
    ...
    jwt: text('jwt'), // JWT token for accessing this email address
    createdAt: timestamp('created_at').defaultNow().notNull(),
  },
  (table) => [
    // Query user's email history by creation time (latest first)
    index('idx_email_history_user_created').on(table.userId, table.createdAt),
  ]
);
