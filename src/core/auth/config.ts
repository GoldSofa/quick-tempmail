...
const authOptions = {
  ...
  baseURL: process.env.NODE_ENV === 'production' ? envConfigs.auth_url : 'http://localhost:3000',
  ...
  trustedOrigins: envConfigs.app_url
    ? [
      ...
      (origin: string) => /http:\/\/localhost:\d+/.test(origin),
      // Allow any ngrok domain dynamically
      (origin: string) => origin.endsWith('.ngrok-free.dev'),
    ]
    : [
      (origin: string) => /http:\/\/localhost:\d+/.test(origin),
      (origin: string) => origin.endsWith('.ngrok-free.dev'),
