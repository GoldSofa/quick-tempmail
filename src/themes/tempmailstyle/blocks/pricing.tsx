...
export function Pricing({
  ...
  const [group, setGroup] = useState(() => {
    ...
    // First look for a group named 'monthly' or 'Monthly'
    const monthlyGroup = pricing.groups?.find((g) => g.name?.toLowerCase() === 'monthly');
    // If not found, look for featured group
    const featuredGroup = pricing.groups?.find((g) => g.is_featured);

    return (
      currentItem?.group || monthlyGroup?.name || featuredGroup?.name || pricing.groups?.[0]?.name
    );
  });
  ...
  return (
    ...
    >
      ...
      <div className="container">
        ...
        <div
          className={`mx-auto mt-0 grid w-full gap-6 md:grid-cols-${pricing.items?.filter((item) => !item.group || item.group === group)
            ?.length
            }`}
        >
            ...
            return (
              <Card key={idx} className="relative">
                ...
                <CardHeader>
                  ...
                  {/* Temporarily hide buttons for Free and Premium plans */}
                  {isCurrentPlan ? (
                    <Button
                      variant="outline"
                  ...
                  ) : item.product_id.toLowerCase().includes('free') ? null : (
                    <Button
